...
const time_t TIME_LIMIT = time_t(CLOCKS_PER_SEC * 0.89);
const int Score[5] = {99, 32, 16, 8, 4};
const int DefaultDotLimit = 99; // deprecated

/****** Data Structures and Functions ******/

double sqLOG[1000000], sq[1000000], EXP[100]; // precalculate anything related to sqrt(), log(), exp()
...
struct Board { // Chessoard
	...
	inline void getValidPoints(const stone hand, vector <Point> &ret) { // Complexity : O(N ^ 2) + sort_by_eval()
		...
		if (!aval) lasteval = -50;
		else if (!eavl) lasteval = 50; 
		else lasteval = aval - eavl;
	...
	inline int eval(const stone hand) const { // Complexity : O(N ^ 2)
		...
		for (int i = 0; i < N; i++) {
			for (int j = 0; j < N; j++) if (!g[i][j]) {
				...
				if ((o[1] == 0 || o[1] < c[1] || e) && (!o[2]) && (c[1] + e)) aval++;
				if ((o[2] == 0 || o[2] < c[2] || e) && (!o[1]) && (c[2] + e)) eavl++;
			}
		}
		if (hand == WHITE) swap(aval, eavl);
		if (!aval) lasteval = -50;
		else if (!eavl) lasteval = 50; 
		else lasteval = aval - eavl;
		return lasteval;
	...
	inline double simulation(const Node *x) {
		...
		while (T--) {
			...
			while (true) {
				...
				if (cnt++ > 9) {
					double res = Rate(tmp.eval(Switch(hand)));
					if (hand == x->hand) res = 1 - res;
					Y += res;
					break;
				}
				hand = Switch(hand);
...
inline void init() {
	...
	for (int i = 1; i < 1000000; i++) sqLOG[i] = (double)sqrtl(log(i));
	for (int i = 1; i < 1000000; i++) sq[i] = (double)sqrtl(i);
}
...
inline pair<int, int> Action() {
	...
	Point ans;
	if (choice.size()) {
		ans = (10 < choice.size() && choice.size() < 26) ? uct.Action() : alphabeta.Action(); // divide-and-conqure on algorithms
	}
	else {  // Never say die!
		for (int i = 0; i < N; i++) {
